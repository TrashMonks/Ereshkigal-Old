module.exports = {
  ...
  async execute(message, args, client) {
    ...
    let content = '__**Pronoun Role Usage (Delver only):**__\n';
    ...
    const membersWithPronounRoles = await message.guild.members.cache.filter(u => u.roles.cache.find(r => pronounRoles.find(pronounRole => pronounRole === r )) && u.roles.cache.find(r => r.name === 'Delver') && !u.user.bot).size;
    const membersWithoutPronounRoles = await message.guild.members.cache.filter(u => !u.roles.cache.find(r => pronounRoles.find(pronounRole => pronounRole === r )) && u.roles.cache.find(r => r.name === 'Delver') && !u.user.bot).size;
    const totalRolesApplied = pronounRoles.reduce((prevVal, role) => prevVal + role.members.size, 0);
    const selectedMembers = await message.guild.members.cache.filter(u => u.roles.cache.find(r => u.roles.cache.find(r => r.name === 'Delver') && !u.user.bot)).size;
    ...
    for (curRole of pronounRoles) {
      ...
      const curRoleSize = curRole.members.filter(user => user.roles.cache.find(r => r.name === 'Delver')).size;
      const exclusiveSize = curRole.members.filter(user => !user.roles.cache.some(r => otherRoles.find(otherRole => otherRole === r)) && user.roles.cache.find(r => r.name === 'Delver')).size;
      ...
      // Assign each value to an object to retrieve below, so that we can print them once totalRolesApplied is done calculating
        ...
        content +=
      `> **${curRole.name}**: ${curRoleSize} ${members} (${round(curRoleSize / membersWithPronounRoles)}%)
